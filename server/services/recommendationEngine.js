...
class RecommendationEngine {
  ...
  extractPositiveAspects(pillar, responses, currentScore) {
    ...
    // Strategic insights based on pillar and scores
    const strategicInsights = {
      platform_governance: {
        highScore: 'Strong governance foundation reduces risk and accelerates future AI/ML initiatives',
        smallGap: 'Well-positioned for rapid advancement with minimal effort',
        baseline: 'Platform ready for Unity Catalog migration with clear ROI path'
      },
      data_engineering: {
        highScore: 'Reliable data pipelines enable real-time analytics and reduce firefighting',
        smallGap: 'Quick wins available through automation and monitoring enhancements',
        baseline: 'Foundation in place for Delta Live Tables adoption'
      },
      analytics_bi: {
        highScore: 'Self-service capabilities reduce analyst bottlenecks by 50%+',
        smallGap: 'Positioned to unlock business user productivity with minimal investment',
        baseline: 'Ready for AI/BI and serverless compute adoption'
      },
      machine_learning: {
        highScore: 'Mature MLOps accelerates model deployment from weeks to days',
        smallGap: 'Feature reuse and automation will multiply team productivity',
        baseline: 'MLflow foundation enables rapid experimentation scaling'
      },
      generative_ai: {
        highScore: 'GenAI readiness positions you ahead of 70% of enterprises',
        smallGap: 'Data infrastructure supports immediate GenAI application development',
        baseline: 'Clear path to POC with Databricks GenAI capabilities'
      },
      operational_excellence: {
        highScore: 'Strong CoE and adoption drive 3x faster value realization',
        smallGap: 'Platform investment showing clear ROI and user satisfaction',
        baseline: 'Enablement programs will unlock hidden platform capabilities'
      }
    };
    
    const insights = strategicInsights[pillarId] || strategicInsights.platform_governance;
    ...
    // Always add a strategic insight first
    if (strengths.length > 0) {
      const topStrength = strengths[0];
      if (topStrength.current >= 4) {
        positives.push(insights.highScore);
      } else if (topStrength.gap <= 1) {
        positives.push(insights.smallGap);
      } else {
        positives.push(insights.baseline);
      }
    } else {
      positives.push(insights.baseline);
    }
    
    // Add specific dimensional strengths
  ...
  extractChallenges(technicalPains, businessPains, responses, pillar) {
    ...
    const impactMap = {
      ...
      'poor_isolation': 'Teams working in silos duplicate work, missing 40% of reuse opportunities',
      'data_quality': 'Data trust issues cause 60% of reports to be questioned or ignored by executives',
      'slow_pipelines': 'Batch-only processing delays insights by days while competitors act in real-time',
      'manual_processes': 'Cannot use Delta Live Tables or Workflows—manual pipeline maintenance wastes $600K-1.2M/year in engineering time',
      'deployment_issues': 'No CI/CD with Databricks Asset Bundles means 6-8 week deployment cycles vs 1-2 weeks with automation',
      'monitoring_gaps': 'Missing Lakehouse Monitoring means data quality issues found by business users, not proactive alerts—$500K+ annual impact',
      'security_concerns': 'Vulnerabilities expose organization to breaches averaging $4.5M in costs',
      'performance_issues': 'Cannot use Serverless SQL or Photon acceleration—5-10 min queries drive users to Excel, wasting $400K platform investment',
      'scalability_limits': 'Missing autoscaling and serverless compute blocks concurrent user growth and costs 3x more in over-provisioned clusters',
      'integration_complexity': 'No Unity Catalog federation or Partner Connect—each new data source takes 4-8 weeks vs days with native integrations',
      ...
      'change_tracking': 'No change history makes troubleshooting take 5x longer than necessary',
      ...
      'metadata_gaps': 'No Unity Catalog search or AI-generated documentation—analysts waste 8-12 hours/week finding data vs minutes with discovery tools',
      'performance_impact': 'Missing Query Profile and Photon means 5x slower queries than optimized—analyst productivity down 40-50%',
      'security_gaps': 'No row/column-level security or dynamic views blocks secure data sharing—cannot monetize data products worth $3-8M',
      ...
      'high_costs': 'Unoptimized compute wastes 40-60% of cloud budget without business value',
      'slow_insights': 'Stale data means decisions lag market reality by weeks, missing opportunities',
      'team_bottlenecks': 'Analyst dependencies create week-long queues for basic business questions',
      'quality_issues': 'Poor data quality results in $15M+ annually in bad decisions and rework',
      'skill_gaps': 'Team cannot use Unity Catalog, DLT, or MLflow features—wasting $300K-600K/year in unused license value and manual workarounds',
      'compliance_risks': 'Missing audit capabilities expose organization to SOC2/GDPR violations averaging $250K-2M in fines and remediation',
      'low_adoption': 'Platform adoption under 30% means $500K-1M/year wasted on licenses while teams use Excel and legacy tools',
      'missed_opportunities': 'Cannot leverage Databricks AI/BI, Genie, or Mosaic AI—missing 40-60% productivity gains competitors already captured',
      'competitive_disadvantage': 'Competitors deploy models in 2-4 weeks using MLOps automation while you take 4-6 months with manual processes',
      'user_frustration': 'Poor user experience drives talent to competitors with better tools',
      'business_disruption': 'Unplanned downtime costs $300K+ per hour in lost revenue and productivity',
      'operational_inefficiency': 'Process inefficiency wastes $2M+ annually in unnecessary manual work',
      'reputation_damage': 'Quality incidents erode customer trust and damage brand reputation',
      'resource_waste': 'Inefficient resource usage increases TCO by 200-300% above optimized baseline',
      'team_burnout': 'Constant firefighting drives 40% higher attrition among top performers',
      'inefficient_spending': 'No cost visibility means departments overspend by 150-200% of efficient baseline',
      'roi_unclear': 'Inability to demonstrate value puts future platform funding at risk',
      'analyst_frustration': 'Tool complexity limits self-service, keeping 80% of requests with IT',
      'tool_proliferation': 'Tool sprawl increases license costs 300% while fragmenting expertise',
      ...
      'security_breaches': 'Data breaches average $4.5M in costs plus immeasurable reputation damage',
      'duplicate_efforts': 'Without Feature Store, teams rebuild same ML features 3-5 times—wasting $400K-800K/year in redundant engineering',
      'compliance_risk': 'No model lineage or governance blocks regulated AI use cases worth $5-10M in potential revenue',
      'knowledge_loss': 'No MLflow experiment tracking means lost model knowledge costs 4-8 weeks per model recreation',
      'scaling_challenges': 'Cannot use Model Serving or automated retraining—stuck at 2-5 production models vs competitors with 30-50+',
      ...
      'slow_development': 'No Vector Search or Model Serving for LLMs blocks GenAI apps—missing customer experience innovations worth $10-20M',
      'competitive_gap': 'Competitors launched RAG/GenAI features 15-24 months ago, capturing market share while you evaluate POCs',
      'quality_variance': 'Inconsistent quality creates unpredictable outcomes and erodes stakeholder trust',
      'technical_debt': 'Technical debt accumulation will require $5-10M remediation in 2-3 years',
      ...
      'reactive_mode': 'Fire-fighting prevents strategic work, limiting innovation to <20% of capacity',
      ...
      'initiative_failures': '60% of new initiatives fail due to lack of platform expertise and support',
      'talent_retention': 'Outdated tools and practices drive best engineers to competitors',
      'inconsistent_quality': 'Quality variability makes outcomes unpredictable and planning impossible',
      'missed_synergies': 'Isolated teams duplicate work that costs $1M+ annually in redundancy',
      'limited_scaling': 'Current model cannot scale beyond 2-3 teams without exponential complexity',
      'hiring_challenges': 'Skills gap makes it impossible to hire at the pace business demands',
      'dependency_bottlenecks': 'Expert dependencies create queues that delay projects by months',
      'slow_productivity': 'New team members take 6+ months to become productive vs 4-6 weeks industry average',
      'competitive_lag': 'Being 12-18 months behind in platform capabilities limits competitive response',
      'poor_roi': 'Platform investment delivers <30% of expected value due to poor enablement',
      'budget_justification': 'Cannot demonstrate value, putting future investment at risk of cuts',
      'budget_pressures': 'Unoptimized spending forces cuts to strategic initiatives',
      'value_erosion': 'Legacy spending patterns waste 40-60% of budget on low-value activities',
      'platform_underutilization': 'Using <30% of platform capabilities means wasting 70% of license spend',
      'roadmap_misalignment': 'Platform evolution misaligned with business needs wastes investment',
      'adoption_resistance': 'Change resistance limits new capability adoption to <20% of target audience'
    };
    
    // Get all questions to analyze gap magnitude
    const questions = [];
    if (pillar.dimensions) {
      pillar.dimensions.forEach(dimension => {
        if (dimension.questions) {
    ...
    // Map pain points to business impacts (limit to 4 for visual balance)
    const topPains = [...new Set([...technicalPains, ...businessPains])].slice(0, 10);
